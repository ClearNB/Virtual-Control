...
class form_generator implements form_in {
     ...
     * 定義確認
     * @var array   $gen_data	    
     * @var string  $select_data    <SELECT>使用時の取り込みデータです
     * @var string  $fdata	    <SELECT>使用時の取り込みデータです
     * @var array   $data	    フォームデータを配列で格納されています
     * @var string  $id		    フォームデータに対する識別用のIDです
     */
     ...
     */
    function SubTitle($title, $caption, $icon, $darkBack = false, $badgetext = '') {
	$back_text = '';
	$b_text = '';
	if($darkBack) { $back_text = 'class = "orange"'; }
	if($badgetext) {
	    if($darkBack) {
