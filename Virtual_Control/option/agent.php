...
$fm_ag_cr->SubTitle('エージェント作成', '以下の情報を入力してください', 'plus-square', false, '1: 入力');
	...
	<?php echo $loader->footerS(true) ?>
		...
		ajax_dynamic_post_toget('../scripts/agent/agent_get.php').then(function (data) {
			...
			case 0:
			    ...
			    break;
			case 1:
			    animation('data_output', 400, fm_pg_fl);
			    break;
		    }
		});
	    }
	    ...
	    $(document).on('click', '#bt_sl_nx, #bt_sl_bk', function () { //エージェント作成（MIB選択）
		switch ($(this).attr('id')) {
		    case "bt_sl_nx":
	    ...
	    $(document).on('click', '#bt_ip_nx, #bt_ip_bk', function () { //エージェント編集（ホストアドレス）
		switch ($(this).attr('id')) {
		    case "bt_ip_nx":
	    ...
	    $(document).on('click', '#bt_mb_nx, #bt_mb_bk', function () { //エージェント編集（MIB選択）
		switch ($(this).attr('id')) {
		    case "bt_mb_nx":
			animation('data_output', 400, fm_ag_ed);//遷移先未設定
			break;
		...
		var count = ("#sl_ag input:checkbox:checked").length;
		if (count === 1) {
